...
namespace CristalImb. Business.Business
{
    public class InmuebleService : IInmuebleService
    {
        ...
        {
            return await _context.inmuebles.Where(s => s.oferta == true && (s.IdEstadoInm == 3 || s.IdEstadoInm == 1)&& s.Estado == true && s.NombreArchivo != null).ToListAsync();
        }

        public async Task<IEnumerable<Inmueble>> ObtenerListaInmueblesArriendo()
        {
            return await _context.inmuebles.Where(s => s.ServicioInmuebleId == 2 && (s.IdEstadoInm == 3 || s.IdEstadoInm == 1) && s.Estado == true && s.NombreArchivo != null).ToListAsync();         
        }

        public async Task<IEnumerable<Inmueble>> ObtenerListaInmueblesVenta()
        {
            return await _context.inmuebles.Where(s => s.ServicioInmuebleId == 1 && (s.IdEstadoInm == 3 || s.IdEstadoInm == 1) && s.Estado == true && s.NombreArchivo != null).ToListAsync();
        }
        ...
        public async Task<Inmueble> CodigoExisteEditar(string codigo, int inmuebleId)
        {
            return await _context.inmuebles.FirstOrDefaultAsync(x => x.Codigo == codigo && x.InmuebleId != inmuebleId);
        }
